...
function changeInfoContente(v) {
  ...
  if (v === 0) { hold.style.display = "none"; cont.style.display = "block"; } 
  else { hold.style.display = "block"; cont.style.display = "none"; }
}

async function npsDud(val) {
  let oldVal = parseInt(npsVal.textContent) || 0;
  let newVal = oldVal + val;
  if (newVal < 0) { alert("Não pode ser inferior a 0"); newVal = 0; }
...
function parseInput(text) {
  ...
  raw.forEach(chunk=>{
    ...
    parts.forEach(p=>{
      ...
      let cleaned = p.replace(/\*/g,"").replace(/min(?:ute)?s?/i,"").replace(/m\b/i,"").replace(/,/g,".").replace(/[^0-9.\-]/g,"");
      if(cleaned==="") return;
      let num = Number(cleaned);
      if(isNaN(num)) return;
      if(num%1!==0){ const integerPart = Math.floor(num); const assumedSeconds = Math.round((num-integerPart)*100); num = integerPart + assumedSeconds/60; }
      values.push({ value: num, ignoreIQS });
    });
  });
  return values;
}
...
function showResult(list, res) {
  ...
  const percent = validForIQS ? ((nps/validForIQS)*100).toFixed(1) : 0;
...
observer.observe(tmsSecEl,{childList:true,characterData:true,subtree:true});

calc.addEventListener("click", ()=>{
  const list=parseInput(input.value);
  if(list.length===0){ alert("Nenhum tempo válido encontrado."); return; }
  const res=computeTMA(list);
  showResult(list,res);
});

clear.addEventListener("click", async ()=>{
  input.value=""; npsVal.textContent="0"; results.hidden=true;
  if(window.pywebview){ await window.pywebview.api.guardar({ tma_input:"", nps_val:0 }); }
});

copyBtn.addEventListener("click", async ()=>{
  if(results.hidden){ alert("Nada para copiar."); return; }
  const text = `TMA: ${tmaSecEl.textContent} segundos. ${formulaEl.textContent}`;
  try{ await navigator.clipboard.writeText(text); alert("Resultado copiado."); } 
...
input.addEventListener("keydown",e=>{ if(e.key==="Enter"&&(e.ctrlKey||e.metaKey)) calc.click(); });

usercode.addEventListener("input", e=>{
  let code = e.target.value.trim();
  if(code.length>=6) divBtns.classList.remove("hide"); else divBtns.classList.add("hide");
});
